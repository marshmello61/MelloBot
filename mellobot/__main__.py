...
if Config.ENABLE_ASSISTANTBOT == "ENABLE":
    path = "mellobot/modules/assistant/*.py"
    files = glob.glob(path)
    for name in files:
        with open(name) as f:
            path1 = Path(f.name)
    ...
    sed.info("MelloBot And Assistant Bot Have Been Installed Successfully !")
else:
