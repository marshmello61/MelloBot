...
@fire.on(fire_on_cmd(pattern="cz5$"))
...
@fire.on(fire_on_cmd(pattern="cnd$"))
...
@fire.on(fire_on_cmd(pattern="combogen"))
...
@fire.on(fire_on_cmd(pattern="cvx$"))
...
@fire.on(fire_on_cmd(pattern="cvr$"))
async def vortex(event):
    await event.edit(
        "`Checking Your Combos. This May Take Time Depending On No of Combos.`"
    )
...
@fire.on(admin_cmd(pattern="proxy"))
async def _(event):
    if event.fwd_from:
        return
    await event.edit(
    ...
    if Counter <= 0:
        ...
        await borg.send_file(
            event.chat_id,
            "goood.txt",
            caption=f"**PROXIES CHECKED**\n**GOOD PROXIES: ** {Counter}\n\n**CHECKED BY MelloBot. GET YOUR OWN MelloBot FROM @MELLOLAB**",
        )
        os.remove(escobar)
        os.remove("goood.txt")


@fire.on(fire_on_cmd(pattern="cvt$"))
...
@fire.on(fire_on_cmd(pattern="cab$"))
