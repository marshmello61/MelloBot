...
from ..utils import edit_or_reply, fire_on_cmd, sudo_cmd


@fire.on(fire_on_cmd(pattern="app (.*)"))
@fire.on(sudo_cmd(pattern="app (.*)", allow_sudo=True))
async def apk(event):
...
@fire.on(fire_on_cmd(pattern="appr (.*)"))
@fire.on(sudo_cmd(pattern="appr (.*)", allow_sudo=True))
