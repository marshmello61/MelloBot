...
async def set_afk(afk_e):
    ...
    if string:
        ...
        await afk_e.edit("**Be Right Back!**")
    if user.last_name:
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"I'll be right back. Offline since {afk_since}.\
                        \nReason: {AFKREASON}")
                else:
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"I'll be right back. Offline since {afk_since}.\
                        \nReason: {AFKREASON}")
                else:
