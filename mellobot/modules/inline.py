...
if PMPERMIT_PIC is None:
    WARN_PIC = "https://telegra.ph/file/f0082cfc1dbca61632a33.jpg"
else:
...
if lang == "si":
    ...
    async def inline_handler(event):
        ...
        result = None
        query = event.text
        if event.query.user_id == bot.uid and query.startswith("MelloBot"):
            rev_text = query[::-1]
            ...
            result = builder.article(
                title="Stats",
                text=f"**Showing Stats For {DEFAULTUSER}'s MelloBot** \nNote --> Only Owner Can Check This \n(C) [MelloBot](https://github.com/marshmello61/MelloBot)",
                buttons=[
                    [custom.Button.inline("Show Stats ?", data="terminator")],
                    [Button.url("Developed By", "https://github.com/marshmello61")],
    ...
    async def rip(event):
        ...
        link_s = yt_dl_data
        if event.query.user_id != bot.uid:
            text = f"Please Get Your Own MelloBot And Don't Waste My Resources"
            await event.answer(text, alert=True)
            return
        is_it = True
    ...
    async def sed(event):
        ...
        buttons = paginate_help(0, CMD_HELP, "helpme")
        sed = f"""MelloBot Modules Are Listed Here !\n
        ...
        if event.query.user_id == bot.uid and query.startswith("MelloBot"):
            ...
            result = builder.article(
                ...
                buttons=[
                    ...
                    [
                        Button.url(
                            "Repo Here", "https://github.com/marshmello61/MelloBot"
                        )
                    ],
                    [Button.url("Join Channel ❤️", "t.me/https://t.me/MelloHub"],
                ],
            )
            await event.answer([result])
        elif event.query.user_id == bot.uid and query.startswith("**Hello"):
    ...
    async def on_plug_in_callback_query_handler(event):
        if not event.query.user_id == bot.uid:
            sedok = "Who The Fuck Are You? Get Your Own MelloBot ."
            await event.answer(sedok, cache_time=0, alert=True)
            return
        plugin_name = event.data_match.group(1).decode("UTF-8")
        if plugin_name in CMD_HELP:
    ...
    async def sed(event):
        ...
        # This Is Copy of Above Code. (C) @SpEcHiDe
        buttons = paginate_help(0, CMD_HELP, "helpme")
        sed = f"""MelloBot Userbot Modules Are Listed Here !\n
...
async def inline_id_handler(event: events.InlineQuery.Event):
    ...
    if event.query.user_id != bot.uid:
        resultm = builder.article(
            title="Not Allowded",
            text=f"You Can't Use This Bot. \nDeploy MelloBot To Get Your Own Assistant, Repo Link [Here](https://github.com/marshmello61/MelloBot)",
        )
