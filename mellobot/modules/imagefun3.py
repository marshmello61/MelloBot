...
async def hehe(event):
    ...
    await event.client.send_file(
        ...
        "danish.jpg",
        force_document=False,
        caption="To Know How this works type .howctoon",
        reply_to=event.reply_to_msg_id,
    )
    await event.delete()
    shutil.rmtree(path)
...
async def hehe(event):
    if not event.reply_to_msg_id:
        await event.reply(
            "Reply to any Image like this :-.rmbw 01;1.5;9; u can have any value instead"
        )
        return
    reply = await event.get_reply_message()
    await event.edit("`Processing...`")
...
async def howitworks(event):
    ...
    hm = await bot.edit_message(
        event.chat.id, ni, "Stage 1 - Collecting The Gray form."
    )
    await asyncio.sleep(3.5)
    sh = await bot.edit_message(event.chat.id, hm, file="c.jpg")
    lo = await bot.edit_message(
        event.chat.id, sh, "Stage 2 - Collecting The Threshold Edges ."
    )
    await asyncio.sleep(3.5)
    ...
    await bot.edit_message(event.chat.id, co, "Output - Merged All Stages.")
    await asyncio.sleep(3.5)
