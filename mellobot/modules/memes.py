...
async def source(e):
    if not e.text[0].isalpha() and e.text[0] not in ("/", "#", "@", "!"):
        await e.edit(
            "Hello User, I Am Using [MelloBot](https://github.com/marshmello61/MelloBot) ! Worth A Try 😌"
        )
...
@fire.on(fire_on_cmd(pattern=r"hola"))
async def hi(event):
    if event.fwd_from:
        return
    await event.edit("╔┓┏╦━╦┓╔┓╔━━╗\n║┗┛║┗╣┃║┃║X X  ║\n║┏┓║┏╣┗╣┗╣╰╯║\n╚┛┗╩━╩━╩━╩━━╝")
...
@fire.on(fire_on_cmd(pattern=r"plus"))
async def hi(event):
    if event.fwd_from:
        return
    await event.edit(
...
@fire.on(fire_on_cmd(pattern=r"yes"))
async def hi(event):
    if event.fwd_from:
        return
    await event.edit(
...
@fire.on(fire_on_cmd(pattern=r"android"))
async def hi(event):
    if event.fwd_from:
        return
    await event.edit(
...
@fire.on(fire_on_cmd(pattern=r"hmm"))
async def hi(event):
    if event.fwd_from:
        return
    await event.edit(
...
@fire.on(fire_on_cmd(pattern=r"happybirthday"))
async def hi(event):
    if event.fwd_from:
        return
    await event.edit(
...
@fire.on(fire_on_cmd(pattern=r"WTF"))
async def hi(event):
    if event.fwd_from:
        return
    await event.edit(
...
@fire.on(fire_on_cmd(pattern=r"lmao"))
async def hi(event):
    if event.fwd_from:
        return
    await event.edit(
...
@fire.on(events.NewMessage(pattern=r"\.spam", outgoing=True))
async def spammer(e):
...
@fire.on(fire_on_cmd(pattern=r"no"))
async def hi(event):
    if event.fwd_from:
        return
    await event.edit(
